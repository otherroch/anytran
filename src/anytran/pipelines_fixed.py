...
def execute_pipeline(args, config):
    ...
    if args.web:
        return _run_web_pipeline(args, runner_config)
    elif args.from_output:
        return _run_output_pipeline(args, runner_config)
    elif args.youtube_url:
        return _run_youtube_pipeline(args, runner_config)
    elif args.rtsp:
        return _run_rtsp_pipeline(args, runner_config)
    elif args.input:
        return _run_file_pipeline(args, runner_config)
    else:
...
def _run_rtsp_pipeline(args, config):
    ...
    runner_config = RunnerConfig(**config)
    if len(args.rtsp) == 1:
        topic = args.mqtt_topic_names[0] if args.mqtt_topic_names else runner_config.mqtt_topic
        run_realtime_rtsp(
            args.rtsp[0],
            runner_config.input_lang,
            runner_config.output_lang,
            ...
            voice_backend=runner_config.voice_backend,
            voice_model=runner_config.voice_model,
            chat_log_dir=runner_config.chat_log_dir,
            window_seconds=runner_config.window_seconds,
            overlap_seconds=runner_config.overlap_seconds,
            timers=runner_config.timers,
            timers_all=runner_config.timers_all,
        ...
        run_multi_rtsp(
            ...
            runner_config.input_lang,
            runner_config.output_lang,
            runner_config.scribe_voice if isinstance(runner_config.scribe_voice, str) else None,
            runner_config.slate_voice if isinstance(runner_config.slate_voice, str) else None,
            runner_config.magnitude_threshold,
            model=runner_config.model,
            verbose=runner_config.verbose,
            mqtt_broker=runner_config.mqtt_broker,
...
def _run_file_pipeline(args, config):
    ...
    if (
        ...
        for i in range(1, looptran_n + 1):
            ...
            # Create RunnerConfig object for the looptran call
            looptran_config = RunnerConfig(**config)
            looptran_config.input_lang = current_input_lang
            looptran_config.output_lang = current_output_lang
            looptran_config.text_translation_target = text_translation_target
            looptran_config.voice_lang = voice_lang
            looptran_config.scribe_text_file = new_scribe_text
            looptran_config.slate_text_file = new_slate_text
            looptran_config.scribe_voice = config["scribe_voice"] if isinstance(config["scribe_voice"], str) else None
            looptran_config.slate_voice = config["slate_voice"] if isinstance(config["slate_voice"], str) else None
            looptran_config.scribe_backend = config["scribe_backend"]
            looptran_config.slate_backend = config["slate_backend"]
            looptran_config.voice_backend = config["voice_backend"]
            looptran_config.voice_model = config["voice_model"]
            looptran_config.voice_match = config["voice_match"]
            looptran_config.dedup = config["dedup"]
            looptran_config.lang_prefix = config["lang_prefix"]
            looptran_config.normalize = config.get("normalize", True)
            looptran_config.slate_no_opt = config.get("slate_no_opt", False)
            looptran_config.batch = args.batch_input_text
            looptran_config.keep_temp = config.get("keep_temp", False)
