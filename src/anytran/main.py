...
"""
    ...
    voice_group.add_argument("--voice-backend", type=str, default="auto", choices=["piper", "gtts", "custom", "fish", "indextts", "auto"], help="TTS backend. 'auto' (default) prefers Piper if available and falls back to gTTS otherwise. Use 'piper' to force Piper TTS, 'gtts' to force Google TTS, 'custom' to use Qwen3-TTS CustomVoice/Base models, 'fish' to use fish-speech (supports fishaudio/s1-mini and fishaudio/fish-speech-1.5 models), or 'indextts' to use IndexTTS (supports IndexTeam/IndexTTS-2 and compatible models; requires --voice-match to supply a speaker prompt; install with: GIT_LFS_SKIP_SMUDGE=1 pip install git+https://github.com/index-tts/index-tts.git && pip install 'anytran[index-tts]').")
