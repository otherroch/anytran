...
def run_multi_rtsp(config):
    ...
    print(f"Starting {len(config.rtsp_urls)} RTSP streams...")
    ...
    if config.mqtt_broker:
        if config.topic_names and len(config.topic_names) == len(rtsp_urls):
            print(f"MQTT output enabled: {config.mqtt_broker}:{config.mqtt_port}")
            for i, topic in enumerate(config.topic_names, 1):
    ...
    if config.scribe_text_file:
        print(f"Scribe text (English) will be saved to: {config.scribe_text_file}")
    if config.slate_text_file:
        print(f"Slate text (translated) will be saved to: {config.slate_text_file}")
    ...
    timing_stats = TimingsAggregator("multi_rtsp") if config.timers else None
    ...
    def worker(rtsp_url, idx):
        ...
        local_scribe_audio_segments = [] if config.output_audio_path else None
        local_slate_audio_segments = [] if config.slate_audio_path else None
        local_capture_voice_segments = [] if config.capture_voice_path else None
        ...
        if config.topic_names and len(config.topic_names) >= idx:
            stream_mqtt_topic = config.topic_names[idx - 1]
        else:
        ...
        try:
            ...
            if local_scribe_audio_segments is not None:
                if len(local_scribe_audio_segments) > 0:
                    ...
                    output_file = (
                        f"{config.output_audio_path.rsplit('.', 1)[0]}_stream{idx}.{config.output_audio_path.rsplit('.', 1)[1]}"
            ...
            if local_slate_audio_segments is not None:
                if len(local_slate_audio_segments) > 0:
                    ...
                    output_file = (
                        f"{config.slate_audio_path.rsplit('.', 1)[0]}_stream{idx}.{config.slate_audio_path.rsplit('.', 1)[1]}"
                        if config.slate_audio_path
                        else None
                    )
                    if output_file:
                        try:
            ...
            if local_capture_voice_segments is not None:
                if len(local_capture_voice_segments) > 0:
                    ...
                    parts = config.capture_voice_path.rsplit('.', 1)
                    output_file = f"{parts[0]}_stream{idx}.{parts[1]}" if len(parts) == 2 else f"{config.capture_voice_path}_stream{idx}"
                    try:
                        output_audio(all_audio, output_file, play=False)
    ...
    try:
        ...
        if scribe_file:
            scribe_file.close()
            print(f"Scribe text file saved: {config.scribe_text_file}", flush=True)
        if slate_file:
            slate_file.close()
            print(f"Slate text file saved: {config.slate_text_file}", flush=True)
